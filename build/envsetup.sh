...
- cout:    Changes directory to the android product output.
...
function cout()
{
    O=$(get_abs_build_var PRODUCT_OUT)
    if [ "$O" ]; then
        cd $(get_abs_build_var PRODUCT_OUT)
    else
        echo "Couldn't locate the output folder.  Try setting OUT."
    fi
}
